...
$query = "SELECT id FROM rsschannel";
$res1 = mysql_query($query) or mysql_die($MySQLError.mysql_error(), false);
while(list($rssId) = mysql_fetch_row($res1))
{
	$query = "SELECT linkMan FROM rsschannel WHERE id='$rssId'";
	...
	foreach ($urls as $url)
	{
		...
		if (isset($img[0]))
		{
			...
			$query = "DELETE FROM rssimage WHERE idCh='$rssId'";
			mysql_query($query) or mysql_die($MySQLError.mysql_error(), false);
			$query = "INSERT INTO rssimage (`idCh`, `title`, `description`, `link`, `url`, `width`, `height`) VALUES
					('$rssId', '".$img[0][title]."', '".$img[0][description]."', '".$img[0][link]."',
		...
		foreach ($items as $item)
		{
			...
			if (isset($item))
			{
				...
				$query = "SELECT count(*) AS cnt FROM rssitem_all WHERE idCh='$rssId' AND (pubDate='".date("Y-m-d H:i:s",strtotime($item[pubdate]))."' AND (pubDate<>'1970-01-01 02:00:00' OR link='".$item[link]."'))";
				$res = mysql_query($query) or mysql_die($MySQLError.mysql_error(), false);
				...
				if ($cnt == 0)
				{
					...
					$query = "INSERT INTO `rssitem` (`idCh` , `title` , `description` , `link` ,
							...
							VALUES (
							'$rssId' , '$item[title]', '$item[description]', '$item[link]',
