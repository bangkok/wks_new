...
$ident=@mysql_connect($server,$login,$password) or mysql_die("Error connecting to MySQL server.", true);
...
<html>
<head>
<title>Site administrative utility "<?print $site_name?>".</title>
...
<?
	...
	{
		print "No files for download.<br>Create file_to_base (pg/files_en/file_to_base) and place there files for dowload to DB.";
	}
	else
	{
		...
		if (COUNT($file_ar)==0)
		{
			print "No files for download.<br>Create file_to_base adm/file_to_base and place there files for dowload to DB.";
		}
		for ($i=0;$i<COUNT($file_ar);$i++)
		{
			...
			$res = mysql_db_query($base, $sql_query) or mysql_die("Bad MySQL query! ".mysql_error(), false);
